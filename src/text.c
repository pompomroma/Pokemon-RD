...
u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    case RENDER_STATE_HANDLE_CHAR:
        ...
        case CHAR_EXTRA_SYMBOL:
            currChar = *textPrinter->printerTemplate.currentChar++;
            if (currChar == EXTRA_SYMBOL_WIDE)
            {
                // Wide form: the next two bytes are a full glyph id, which
                // reaches the banks above 0x1FF that Hangul needs.
                currChar = *textPrinter->printerTemplate.currentChar++ << 8;
                currChar |= *textPrinter->printerTemplate.currentChar++;
            }
            else
...
static s32 GetStringWidthFixedWidthFont(const u8 *str, u8 fontId, u8 letterSpacing)
{
    ...
    do
    {
        ...
        case CHAR_KEYPAD_ICON:
        case CHAR_EXTRA_SYMBOL:
            // A wide extra symbol carries two extra bytes past the 0xFF marker.
            if (temp == CHAR_EXTRA_SYMBOL && str[strPos + 1] == EXTRA_SYMBOL_WIDE)
                strPos += 3;
            else
                ++strPos;
        default:
...
{
    // Callers measure a string to centre or right-align it and then print the
    // same pointer, which AddTextPrinter localizes. Measuring the untranslated
    // string would lay the translated one out to the wrong width, so the same
    // substitution has to happen here.
    str = GameText_Localize(str);
    {
    bool8 isJapanese;
    int minGlyphWidth;
    ...
    while (*str != EOS)
    {
        ...
        case CHAR_EXTRA_SYMBOL:
            ...
            {
                if (*++str == EXTRA_SYMBOL_WIDE)
                {
                    u16 wide = *++str << 8;
                    wide |= *++str;
                    glyphWidth = func(wide, isJapanese);
                }
                else
                {
...
u8 RenderTextHandleBold(u8 *pixels, u8 fontId, u8 *str, int a3, int a4, int a5, int a6, int a7)
{
    ...
    do
    {
        ...
        case EXT_CTRL_CODE_BEGIN:
            ...
            break;
        case CHAR_EXTRA_SYMBOL:
            if (str[strPos + 1] == EXTRA_SYMBOL_WIDE)
                strPos += 3;
            else
                ++strPos;
            break;
